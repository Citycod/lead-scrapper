...
| **SerpApi** (free tier) | 100 free searches/month — scrapes Google Maps |
...
3. You get **100 free searches/month** (more than enough to start)
...
# Fill in: SERPAPI_KEY, GOOGLE_SHEET_ID, TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID
```
...
4. Open the **SerpApi Google Maps** node → verify the API key expression
...
9:00 AM WAT
  ...
  → Loops through 10 niches × 2 cities = 20 SerpApi calls
  → Filters businesses with NO website
  → Deduplicates against existing leads
  → Appends new leads to Google Sheets
  → Sends Telegram notification with count + sheet link
```
...
| Tier | Monthly Cost | Searches | Leads |
|------|-------------|----------|-------|
| **Free** | ₦0 | 100/month | ~300–600 |
| **Growth** | ~$50 (₦40K) | 5,000/month | ~15,000–30,000 |

> Start free. Upgrade SerpApi only when you're making money from the leads.
